...
namespace CSPGF.Parse
{
    class ParseState2
    {
        ...
        public void extractTrees()
        {
            ...
            if (fids != null)
            {
                for(int fid0 = fids.FirstFID; fid0 < fids.LastFID; fid0++)
                {
                    List<ProductionApply> rules = this.chart.expandForest(fid0);
                    foreach(ProductionApply pa in rules)
                    {
                        List<int> labels = new List<int>();
                        foreach(Symbol[] lsym in pa.Function.Sequences)
                        {
                            foreach(Symbol sym in lsym)
                            {
                                // TODO check if we can skip the other symbols
                                if (sym is SymbolCat)
                                {
                                    labels.Add(((SymbolCat)sym).Label);
                                }
                                else if (sym is SymbolLit)
                                {
                                    labels.Add(((SymbolLit)sym).Label);
                                }
                            }
                        }
                        ...
                        {
                            var fid = this.chart.lookupPC(fid0, label, 0);
                            if (fid.HasValue)
                            {
                                var arg_ts = go(fid.Value, forest);
                                foreach(var i in arg_ts)
                                {
                                    bool isMember = false;
                                    foreach(var j in trees)
                                    {
                                        if (i.Equals(j))
                                        {
                                            isMember = true;
...
function go(fid) {
    if (fid < totalFIds) {
      ...
      for (var j in rules) {
        ...
        if (rule.id == "Const") {
          trees.push(rule.lit); //???                       ****** HÄR ÄR JAG :D
        } else {        
          var arg_ix = new Array();
          var arg_ts = new Array();
          for (var k in rule.args) {
            arg_ix[k] = 0;
            arg_ts[k] = go(rule.args[k].fid);   // ah >_>
          }
